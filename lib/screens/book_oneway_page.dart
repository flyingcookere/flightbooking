...
class _OneWayBookingPageState extends State<OneWayBookingPage> {
  ...
  Future<void> _showPassengerPicker() async {
    ...
    Widget buildRow(String label, String sub, int count, Function(int) onChange) {
      ...
      return Padding(padding: const EdgeInsets.symmetric(vertical: 10), child: Row(children: [
        ...
        IconButton(onPressed: count <= 0 ? null : () => onChange(count - 1), icon: Icon(Icons.remove_circle_outline, color: count <= 0 ? Colors.grey : Colors.blue)),
        ...
        IconButton(onPressed: isIncDisabled ? null : () => onChange(count + 1), icon: Icon(Icons.add_circle_outline, color: isIncDisabled ? Colors.grey : Colors.blue)),
      ]));
    }
  ...
  Future<void> handleBooking(String status, String method) async {
    if (selectedFlightDocId == null || totalPassengers <= 0 || paymentMethod == null) return;
    ...
    try {
      ...
      setState(() { 
        ...
        selectedFlightDocId=null; maxSeatsAvailable=0; paymentMethod = null;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    bool canProceed = selectedFlightDocId != null && totalPassengers > 0 && paymentMethod != null;
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        if (originCity != null && destinationCity != null && departureDate != null) ...[
        ...
        Row(children: [
              Expanded(child: OutlinedButton(
                  onPressed: (canProceed && !_isBooking) ? () => handleBooking('reserved', paymentMethod!) : null,
             ...
             const SizedBox(width: 16),
              Expanded(child: ElevatedButton(
                  onPressed: (canProceed && !_isBooking) ? () => handleBooking('confirmed', paymentMethod!) : null,
