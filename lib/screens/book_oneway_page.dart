...
class _OneWayBookingPageState extends State<OneWayBookingPage> {
  ...
  CollectionReference getFlightsCollection() {
    String currentRoute =
        '${originCity ?? "MNL"}-to-${destinationCity ?? "CEBU"}';
    return _firestore
  ...
  Future<void> _showPassengerPicker() async {
    ...
    Widget buildRow(
        String label, String sub, int count, Function(int) onChange) {
      int currentTotal =
          tempAdult + tempChild + tempInfant + tempPWD + tempOFW + tempSenior;
      ...
      return Padding(
        ...
        child: Row(
          children: [
            ...
            IconButton(
              onPressed: count <= 0 ? null : () => setState(() => onChange(count - 1)),
            ...
            IconButton(
              onPressed: isIncDisabled ? null : () => setState(() => onChange(count + 1)),
  ...
  Widget _buildLocationInput(
      {required String label, required String? code, required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
      child: Padding(
                ...
                children: [
                  Expanded(
                      child: _buildSelector(
                          "Passengers", "$totalPassengers", _showPassengerPicker)),
                  const VerticalDivider(
                      ...
                      endIndent: 10),
                  Expanded(
                      child: _buildSelector("Class", flightClass, _showClassPicker)),
                ],
              ),
            ),
          ),
