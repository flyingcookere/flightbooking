...
class _BookRoundTripPageState extends State<BookRoundTripPage> { 
  ...
  Future<void> _pickReturnDate() async { 
    ...
    final picked = await showDatePicker(
      context: context, 
      initialDate: returnDate ?? (departureDate ?? now).add(const Duration(days: 1)), 
      firstDate: departureDate ?? now.add(const Duration(days: 1)), 
          ...
          return Padding(
            ...
            child: Row(
              children: [
                ...
                IconButton(
                  onPressed: count <= 0 ? null : () => setModal(() => onChange(count - 1)), 
                  icon: Icon(Icons.remove_circle_outline, color: count <= 0 ? Colors.grey : Colors.blue)
                ), 
                SizedBox(
                  width: 20, 
                ...
                IconButton(
                  onPressed: isIncDisabled ? null : () => setModal(() => onChange(count + 1)), 
                  icon: Icon(Icons.add_circle_outline, color: isIncDisabled ? Colors.grey : Colors.blue)
                )
              ]
            )
          ); 
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  const Text('Select Passengers', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), 
              ...
              Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      buildRow('Adult', '12y+', tempAdult, (v) => tempAdult = v), 
                      buildRow('Child', '2y-11y', tempChild, (v) => tempChild = v), 
                      buildRow('Person with Disability', '', tempPWD, (v) => tempPWD = v), 
                      buildRow('Infant', '16d-23m', tempInfant, (v) => tempInfant = v), 
                      buildRow('Overseas Filipino Worker', '', tempOFW, (v) => tempOFW = v), 
                      buildRow('Senior Citizen', '60y+', tempSenior, (v) => tempSenior = v) 
                    ]
                  )
                )
              ), 
              ElevatedButton(
                onPressed: () => Navigator.pop(ctx, {'adult': tempAdult, 'child': tempChild, 'pwd': tempPWD, 'infant': tempInfant, 'ofw': tempOFW, 'senior': tempSenior}), 
        ...
        builder: (ctx2, setModal) => Container(
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: ListView(
                  children: classes.map((c) => RadioListTile(title: Text(c), value: c, groupValue: tempClass, onChanged: (v) => setModal(() => tempClass = v!))).toList()
                )
              ), 
              ElevatedButton(
  ...
  void selectFlightLeg(String docId, int seats, String type) { 
    setState(() { 
      if (type == 'outbound') { 
        selectedOutboundFlightDocId = docId; 
        maxSeatsAvailableOutbound = seats; 
      } else { 
        selectedReturnFlightDocId = docId; 
        maxSeatsAvailableReturn = seats; 
      } 
    }); 
  } 
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Text(day, style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold)), 
                  const SizedBox(width: 12), 
                  Text(month, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600))
                ]
              )
            ]
          )
      ...
      builder: (context, snapshot) { 
        ...
        final filteredDocs = allDocs.where((doc) { 
          ...
          if (data['date'] is Timestamp && requiredDate != null) { 
            ...
            return flightDate.year == requiredDate.year && flightDate.month == requiredDate.month && flightDate.day == requiredDate.day; 
          } 
          return false; 
        }).toList(); 
